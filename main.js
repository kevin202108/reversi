let reversi = {
    ...
    move : function(r,c){
        let rInc2,cInc2;  //延伸檢查增加量
        let isValiable = 5;     //紀錄先相同 還是先不同
        for(let rInc = -1; rInc < 2; rInc ++){  //繞周圍一圈檢查
            for(let cInc = -1; cInc < 2; cInc ++){
                let cAround = c + cInc + 1;
                let rAround = r + rInc + 1;
                if(stones[rAround][cAround] != 0 && this.stones[r+1][c+1] != this.stones[rAround][cAround]){  //如果周圍有不ㄧ樣的棋 => 同向向外延伸檢查
                    // 將不同的列為待翻
                    let cCheck = c + this.cInc2 + 1;
                    let rCheck = r + this.rInc2 + 1;

                    for(let i = 0; i < 8; i ++){
                        cInc2 += cInc;  //將Inc2＋Inc
                        rInc2 += rInc;
                        if(this.stones[rCheck][cCheck] != 0 && this.stones[rCheck][cCheck] != this.stones[r+1][c+1]){ //如果延伸有棋且不＝下的
                            this.stones[rCheck][cCheck] += 5;  //將那顆列為待翻
                    ...
                    if(isValiable != 18){ //不可以翻
                        ...
                        for(let c = 0; c < 8; c ++){
                            for(let r = 0; r < 8; r ++){  
                                if(stones[r+1][c+1] == 6){  //不可翻黑改回白
                                    stones = 2;
                                }else if(stones[r+1][c+1] == 7){    //不可翻白改回黑
                                    stones = 1;
                                }
                            }
                        }
